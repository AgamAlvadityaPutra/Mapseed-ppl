...
<main
    ...
    <form class="w-4/5 mx-12 flex flex-col bg-white p-8 rounded-xl gap-2" enctype="multipart/form-data"
        action="/ubah-program/{{ $program->id }}" method="POST">
        <h1 class="text-[25px] font-bold">Form Mengubah Data Program Agrikultur</h1>
        <hr class="w-full">
        @csrf
        ...
        <textarea name="informasi_program" id="informasi_program" class="py-2 px-3 border-2 rounded-md w-full resize-none"
            cols="30" rows="5" placeholder="Informasi Program">{{ old('informasi_program') ? old('informasi_program') : $program->informasi_program }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-2">
            ...
            <div class="">
                ...
                <img src="/storage/{{ $program->foto_program }}"
                    class="w-full h-44 object-cover border-4 border-dashed border-yellow-400"
