...
<main class="mt-2 flex flex-col gap-4 px-4 pb-4 w-full">
    <form action="/tambah-benih" method="POST" class="bg-white rounded-xl" enctype="multipart/form-data">
        @csrf
        <h1 class="p-4 pb-2 text-xl font-bold">Form Membuat Data Benih</h1>
        <hr>
        <section class="p-4 pt-2 flex flex-col gap-1">
            ...
            <input name="nama_produk" type="text" class="px-2 py-1 border border-slate-500 rounded-sm w-full"
                value="{{ old('nama_produk') }}" placeholder="Benih Cabai Merah Keriting Hibrida 'Super Hot'">
            ...
            <input name="nama_varietas" type="text" class="px-2 py-1 border border-slate-500 rounded-sm w-full"
                value="{{ old('nama_varietas') }}" placeholder="Super Hot">
            ...
            <textarea name="deskripsi" class="px-2 py-1 border border-slate-500 rounded-sm w-full resize-none" cols="30"
                rows="5"
                placeholder="Benih Cabai Merah Keriting Hibrida 'Super Hot' merupakan varietas unggul yang menghasilkan cabai merah keriting dengan rasa pedas yang luar biasa. Diformulasikan khusus untuk memberikan panen melimpah dan tahan terhadap hama penyakit.">{{ old('deskripsi') }}</textarea>
            ...
            <input name="berat_produk" type="text" class="px-2 py-1 border border-slate-500 rounded-sm w-full"
                value="{{ old('berat_produk') }}" placeholder="10 gram">

            <h2 class="font-semibold mt-1">Nomor Sertifikasi</h2>
            <input name="nomor_sertifikasi" type="text" value="{{ old('nomor_sertifikasi') }}"
            ...
            <input name="masa_berlaku_produk" type="text" value="{{ old('masa_berlaku_produk') }}"
            ...
            <input name="informasi_musim_tanam" type="text" value="{{ old('informasi_musim_tanam') }}"
