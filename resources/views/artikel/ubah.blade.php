...
<main
    ...
    <form class="w-4/5 mx-12 flex flex-col bg-white p-8 rounded-xl gap-2" enctype="multipart/form-data"
        action="/ubah-artikel/{{ $artikel->id }}" method="POST">
        <h1 class="text-[25px] font-bold">Form Mengubah Data Artikel</h1>
        <hr class="w-full">
        @csrf
        ...
        <input type="text" name="kata_kunci" id="kata_kunci" class="py-2 px-3 border-2 rounded-md w-full"
            placeholder="Kata Kunci" value="{{ old('kata_kunci') ? old('kata_kunci') : $artikel->kata_kunci }}" />
        <label for="kesimpulan">Kesimpulan</label>
        <textarea name="kesimpulan" id="kesimpulan" class="py-2 px-3 border-2 rounded-md w-full resize-none" cols="30"
            rows="3" placeholder="Kesimpulan">{{ old('kesimpulan') ? old('kesimpulan') : $artikel->kesimpulan }}</textarea>
        <div class="grid grid-cols-2 gap-2">
            <label class="cursor-pointer relative">
                <h2 class="my-1">Foto</h2>
                <input name="foto" id="foto" type="file" class="w-0 h-0 opacity-0 absolute top-0 left-0"
                    accept=".jpg,.jpeg,.png">
                <div
            ...
            <div class="">
                ...
                <img src="/storage/{{ $artikel->foto }}"
                    class="w-full h-44 object-cover border-4 border-dashed border-yellow-400" id="foto_preview">
            </div>
        </div>
        ...
        <div class="flex gap-4 mt-4">
            ...
            <a href="/artikel/{{ $artikel->id }}" class="bg-red-500 text-white px-8 py-2 rounded-md">
                Batal
            </a>
        </div>
    </form>
